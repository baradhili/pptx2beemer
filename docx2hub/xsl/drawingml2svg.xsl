  ...
  <xsl:template match="svg:svg" mode="wml-to-dbk" xmlns="http://docbook.org/ns/docbook"
    ...
    <inlinemediaobject><imageobject><imagedata>
      <!-- anchored drawings: keep the anchor position and extent so downstream
           renderers can place the converted vector art absolutely. The anchor
           context is gone by now (the AlternateContent was replaced by this svg
           in add-props), so the attributes are read from the svg itself, where
           add-props remembered them -->
      <xsl:for-each select="(ancestor::w:drawing/wp:anchor
                             [wp:positionH/wp:posOffset][wp:positionV/wp:posOffset][wp:extent][1],
                             self::svg:svg)[1]">
        <xsl:choose>
          <xsl:when test="self::wp:anchor">
            <xsl:attribute name="css:position-left"
          ...
          <xsl:otherwise>
            <xsl:copy-of select="@css:position-left, @css:position-top, @css:width, @css:height"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:for-each>
      <xsl:copy-of select="."/>
  ...
  <xsl:template match="*[a:graphicData]" mode="d2s:default">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      ...
      <g fill="none" stroke="black">
        <!-- no debug background rect: converted art keeps its own fills and
             renders on transparent ground (a page-sized rect painted a green
             page behind the real shapes) -->
...
<!--    <xsl:message select="'aaaaaaaaaaaa ', $align-y"></xsl:message>-->
    ...
    <xsl:for-each select=".//a:custGeom">
      ...
      <!-- a group child's declared ext lives in the group's child coordinate
           space: scale it by the same ext/chExt factor as the position, so the
           drawn size (and the path scaling via the tunnelled xfrm) matches the
           on-page size -->
      <xsl:variable name="child-scale-x" as="xs:double"
                    select="if (exists($group-xfrm/a:chExt/@cx) and xs:integer((for $v in $group-xfrm/a:chExt/@cx[. castable as xs:integer] return $v, 1)[1]) gt 0)
                            then xs:integer((for $v in $group-xfrm/a:ext/@cx[. castable as xs:integer] return $v, 1)[1])
                                 div xs:integer((for $v in $group-xfrm/a:chExt/@cx[. castable as xs:integer] return $v, 1)[1])
                            else 1e0"/>
      <xsl:variable name="child-scale-y" as="xs:double"
                    select="if (exists($group-xfrm/a:chExt/@cy) and xs:integer((for $v in $group-xfrm/a:chExt/@cy[. castable as xs:integer] return $v, 1)[1]) gt 0)
                            then xs:integer((for $v in $group-xfrm/a:ext/@cy[. castable as xs:integer] return $v, 1)[1])
                                 div xs:integer((for $v in $group-xfrm/a:chExt/@cy[. castable as xs:integer] return $v, 1)[1])
                            else 1e0"/>
      <xsl:variable name="draw-ext-x" as="xs:integer"
                    select="xs:integer(round($child-ext-x * $child-scale-x))"/>
      <xsl:variable name="draw-ext-y" as="xs:integer"
                    select="xs:integer(round($child-ext-y * $child-scale-y))"/>
      <xsl:variable name="child-xfrm" select="current()/../a:xfrm" as="element(a:xfrm)?"/>
      <xsl:variable name="scaled-xfrm" as="element(a:xfrm)">
        <a:xfrm>
          <xsl:copy-of select="$child-xfrm/@*"/>
          <xsl:copy-of select="$child-xfrm/a:off"/>
          <a:ext cx="{$draw-ext-x}" cy="{$draw-ext-y}"/>
        </a:xfrm>
      </xsl:variable>
      <g transform="{d2s:transform($shape-position-x, $shape-position-y, $center-x, $center-y, $phi, $flip)}">
        ...
        <xsl:attribute name="d2s:max-x" select="$shape-position-x + $draw-ext-x"/>
        <xsl:attribute name="d2s:max-y" select="$shape-position-y + $draw-ext-y"/>
        ...
        <xsl:apply-templates select="a:pathLst/a:path" mode="d2s:resolve-fmla">
         ...
         <xsl:with-param name="xfrm" select="$scaled-xfrm" as="element(a:xfrm)" tunnel="yes"/>
         <xsl:with-param name="lookup-docs" as="document-node()+" tunnel="yes"
            select="$preset, $d2s:constants"/>
        </xsl:apply-templates>
      </g>
    ...
    <xsl:function name="d2s:pos-y" as="xs:integer">
      ...
      <xsl:choose>
            ...
            <xsl:when test="$relativeFrom = 'paragraph'">
                <xsl:choose>
                    <xsl:when test="matches($align, '\d')">
                      <!-- no paragraph-count × line-pitch term: in floating layouts
                           (documents whose content lives in anchored boxes) the
                           anchor paragraphs have no height in Word's rendering, so
                           margin + offset is where the anchor sits — the same
                           formula the VML text-box and raster-anchor paths use -->
                      <xsl:sequence select="xs:integer($align) + $d2s:sec-layout-map('marginTop') "/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:sequence select="533400"/>
