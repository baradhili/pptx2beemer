...
if [ -z "$JAVA_VERSION" ]; then
    JAVA_VERSION=$($JAVA -version 2>&1 | awk -F'"' '/version/ {split($2,a,"[.+]"); major=a[1]; if(major==1) major=a[2]; print major}')
fi
...
DISTROLIBS="$DISTRO/lib/:$DISTRO/lib/xmlresolver-5.2.2.jar:$DISTRO/lib/xmlresolver-5.2.2-data.jar:$DISTRO/lib/commons-fileupload-1.5.jar:$DISTRO/lib/classindex-3.3.jar:$DISTRO/lib/htmlparser-1.4.jar:$PROJECT_DIR/a9s/common/calabash:$DISTRO/lib/org.restlet-2.2.2.jar:$MAIL_EXT:$DISTRO/lib/tagsoup-1.2.1.jar:$DISTRO/lib/slf4j-simple-1.7.36.jar:$DISTRO/lib/slf4j-api-1.7.36.jar:$DISTRO/lib/commons-io-2.14.0.jar"

CLASSPATH="$ADAPTATIONS_DIR/common/saxon/:$SAXON_JAR:$DIR/saxon/:$RNGVALID_EXT:$DISTRO/xmlcalabash-1.4.1-100.jar:$DISTROLIBS:$EPUBCHECK_EXT:$JAVASCRIPT_EXT:$IMAGEPROPS_EXT:$IMAGETRANSFORM_EXT:$UNZIP_EXT:$MATHTYPE_EXT:$SVN_EXT:$JAF:$CLASSPATH"
